...
| Компонент | Расположение | Спецификация |
|-----------|--------------|--------------|
| REST API | vkr.dzhevelik.com | HTTPS, FastAPI |
| gRPC API | grpc.dzhevelik.com:443 | SSL, FastAPI |
| Neo4j | Сервер (внутренний) | Графовая БД |
| Сервер | dzhevelik.com | 8 vCPU, 32GB RAM, Ubuntu 22.04 |
| Locust | Локальная машина | Windows, Python 3.11.2 |
...
**REST (HTTP/1.1 + JSON):**  
REST использует текстовую сериализацию JSON, что приводит к большему размеру сообщений и дополнительным затратам на парсинг. Однако в данной реализации REST показал лучшую производительность под нагрузкой и более эффективное использование ресурсов при масштабировании. Это указывает на то, что overhead JSON-сериализации компенсируется оптимизацией на уровне HTTP/1.1 и реализации FastAPI.

**gRPC (HTTP/2 + Protobuf):**  
gRPC использует бинарную сериализацию Protocol Buffers, что обеспечивает меньший размер сообщений по сравнению с JSON. Однако протокол HTTP/2 добавляет overhead на управление множественными потоками (multiplexing). В рамках проведенных тестов теоретические преимущества gRPC не реализовались при высокой нагрузке, что может быть связано с конфигурацией HTTP/2 параметров или особенностями конкретной реализации на Python.
